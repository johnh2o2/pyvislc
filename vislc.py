...
# Set window title
root.wm_title("HAT lightcurve visualizer")

# Parse command-line options
parser = argparse.ArgumentParser(description='Visualize HAT lightcurves')
parser.add_argument('--hatlc-file', metavar='lcfile', type=str, nargs='+',
        help='A .gz file path containing a HAT lightcurve')
parser.add_argument('--list-file', action='store', default=None, type=str,
        help='A file path containing a list of HAT ids or lightcurve file paths' ) 
parser.add_argument('--kind', type=str, action='store', default='file', choices=['id','file'],
        help='Specifies whether the list contains HAT IDs or lightcurve file-paths' )
parser.add_argument('--root',type=str,action='store', default=defaults.settings['root-dir'],
        help='Directory containing HAT LC files')
...
command_line_list = args.hatlc_file
...
if not command_line_list is None:
    if kind == 'id':
        command_line_files = [ "%s/%s-hatlc.csv.gz"%(root_dir,ID) for ID in command_line_list ]
    else:
...
if not list_file is None:
    ...
    if kind == "id":
        list_files = [ "%s/%s-hatlc.csv.gz"%(root_dir,ID) for ID in list_file_contents ]
    else:
